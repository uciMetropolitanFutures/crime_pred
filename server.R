...
d <- read.csv("toweb_2019.csv")    # read in data
...
shinyServer(function(input, output) {
  ...
  options = reactiveValues(choose="")   # creates the object 'options'
    ...
    output$hist <- renderPlot({
      ...
      box()
      if(input$year=="2019"){position <- "topright"} else{position <- "topleft"}
    ...
    output$yr_desc <- renderText({
      yr_link = switch(input$year,
                       "2019" = "figures show a predicted crime rate for 2019.  These values are generated by ILSSC based on a model using the trend in crime over the last 15 years.
                       The forecasted value has a degree of uncertainty, but is the model's 'best guess' for the level of crime in 2019.",
                       "Change, 2017-2019" = "presents the difference in the predicted crime rate in 2019 compared to the actual crime rate in 2017.")
